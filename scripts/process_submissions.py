...
def main():
    ...
    meta=load(site/'data/meta.json',{'version':'0.3.0'})
    ...
    for f,d in items:
        ...
        if typ in PUBLIC_EVENT_TYPES:
            e={
                'id':'e_'+uuid.uuid4().hex[:12], 'submission_id':submission_id,
                'type':typ, 'place_id':d.get('place_id'), 'date':d.get('date') or now(),
                'note':str(d.get('note',''))[:1200], 'alias':alias, 'user_id':uid
            }
            if typ=='commitment': e['planned_for']=str(d.get('planned_for',''))[:10] or None
            if typ=='comment': e['reply_to']=str(d.get('reply_to',''))[:80] or None
            if typ=='issue':
                cat=str(d.get('issue_category','other'))
                e['issue_category']=cat if cat in ISSUE_CATEGORIES else 'other'
                e['status']='open'
    ...
    meta['version']=meta.get('version') or '0.3.0'
