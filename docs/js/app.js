...
function openActionDialog(id,type,replyTo=null){
  ...
  if(type==='commitment'){const d=new Date();d.setDate(d.getDate()+1);$('#commitmentDate').value=d.toISOString().slice(0,10);}
...
$('#sendActionBtn').onclick=async()=>{
  ...
  if(type==='commitment'){payload.planned_for=$('#commitmentDate').value;if(!payload.planned_for)return showStatus('Elige una fecha.');}
...
function openIssues(){
  ...
  drawerContent.innerHTML=`<div class="eyebrow">Mapa vivo</div><h1 class="tree-title ui-type">Incidencias</h1><p>Observaciones que conviene revisar. Las etiquetas sirven sólo para ordenar; los detalles están en las notas de cada vecinx.</p>${list.length?list.map(e=>`<article class="issue-list-item"><span class="pill ${e.issue_category==='danger'?'issue-danger':e.issue_category==='dry'?'issue-warn':''}">${esc(issueCategoryLabel(e.issue_category))}</span><h3 class="ui-type">${esc(e.place_id||'Lugar')}</h3><p>${esc(e.note||'Sin nota')}</p><small>${esc(e.alias||'Anónimo')} · ${esc(fmtDateTime(e.date))}</small><div><button class="tiny-btn issue-go" data-place="${esc(e.place_id||'')}">Ver en mapa</button></div></article>`).join(''):'<div class="empty-block">No hay incidencias publicadas todavía.</div>'}`;
  drawer.classList.add('open'); drawerContent.querySelectorAll('.issue-go').forEach(b=>b.onclick=()=>goToPlace(b.dataset.place));
}
...
function openAbout(){
  const version=meta.version||cfg.version||'0.3.0';
  ...
  <p>Los datos iniciales proceden del inventario municipal del Ayuntamiento de Madrid. Ese inventario puede contener errores, omisiones o información desactualizada: la idea es corregir y ampliar el mapa entre todxs.</p>
  <p>Las aportaciones no aparecen necesariamente al instante. El archivo se consolida normalmente una vez al día, y los cambios que modifican el mapa pasan por revisión antes de publicarse.</p>
  <div class="meta-card"><strong class="ui-type">Versión ${esc(version)}</strong><br>Última actualización de datos: ${esc(fmtDateTime(meta.last_update))}<br>Última revisión de la versión: ${esc(fmtDateTime(meta.last_review))}<br>Inventario municipal: actualización ${esc(fmtDate(meta.tree_source?.dataset_updated||'2026-07-27'))}</div>
  <p class="muted">Fuente inicial: <a class="source-link" href="https://datos.madrid.es/dataset/300761-0-arbolado-especies" target="_blank" rel="noopener">Datos Abiertos del Ayuntamiento de Madrid · Arbolado en parques y zonas verdes de Madrid (detalle)</a>. El propio Ayuntamiento indica que el conjunto no incluye todo el arbolado gestionado por otras entidades municipales.</p>
  <p>La intención es que el proyecto pueda crecer con el barrio y, si resulta útil, adaptarse a otros distritos.</p>
  <div class="credit">Proyecto iniciado por <strong>@gildafitnes</strong>.</div>`;
  drawer.classList.add('open');
}
function openGuide(){
  drawerContent.innerHTML=`<div class="eyebrow">En construcción colectiva</div><h1 class="tree-title ui-type">Guía del jardinerx urbanx</h1><p>Esta guía está empezando. Queremos escribirla entre vecinxs y, cuando haga falta, contrastarla con fuentes técnicas fiables.</p><div class="empty-block"><strong>Ideas para desarrollar:</strong><ul class="guide-list"><li>cómo observar si un árbol puede necesitar agua;</li><li>cuándo regar y qué prácticas evitar;</li><li>árbol joven frente a árbol adulto;</li><li>alcorques, suelo y plantas acompañantes;</li><li>qué hacer durante una ola de calor;</li><li>cómo documentar problemas sin dañar el árbol;</li><li>recursos, semillas, actividades y encuentros del barrio.</li></ul></div><p>¿Sabes del tema, quieres corregir algo o proponer una actividad? Usa el <button id="guideSuggest" class="tiny-btn">buzón de sugerencias</button>.</p>`;
...
async function refreshIdentityDialog(){
  ...
  const stateText=id.state==='pending'?'Pendiente de la próxima actualización. Tu código ya está guardado en este navegador.':'Identidad publicada y reconocida.';
  box.innerHTML=`<div class="identity-head"><span class="online-dot ${id.state==='pending'?'pending':''}" title="Identidad activa en este navegador"></span><strong class="ui-type">${esc(id.alias)}</strong></div><p class="muted">${esc(stateText)}</p><div class="drawer-section"><div class="eyebrow">Árboles habituales</div>${habitual.length?`<ul class="profile-list">${habitual.map(([place,n])=>`<li><button type="button" data-goto="${esc(place)}">${esc(place)}</button> <span class="muted">· ${n} riego${n===1?'':'s'}</span></li>`).join('')}</ul>`:'<p class="muted">Aparecerán aquí cuando tengas riegos publicados.</p>'}</div><div class="drawer-section"><div class="eyebrow">Pendientes / compromisos</div>${pending.length?`<ul class="profile-list">${pending.map(e=>`<li><button type="button" data-goto="${esc(e.place_id)}">${esc(e.place_id)}</button><br><span class="muted">${esc(fmtDate(e.planned_for))}${e.note?` · ${esc(e.note)}`:''}</span></li>`).join('')}</ul>`:'<p class="muted">No tienes compromisos futuros publicados.</p>'}</div><div class="drawer-section"><div class="profile-stat"><span>Riegos publicados</span><strong>${mine.filter(e=>e.type==='watering').length}</strong></div><div class="profile-stat"><span>Comentarios</span><strong>${mine.filter(e=>e.type==='comment').length}</strong></div></div><button id="logoutBtn" type="button" class="full ui-type">Salir de esta identidad</button>`;
  box.querySelectorAll('[data-goto]').forEach(b=>b.onclick=()=>{identityDialog.close();goToPlace(b.dataset.goto);});$('#logoutBtn').onclick=()=>{logout();refreshIdentityDialog();refreshIdentityNav();};
}
...
$('#aboutBtn').onclick=openAbout;$('#guideBtn').onclick=openGuide;$('#issuesBtn').onclick=openIssues;$('#suggestBtn').onclick=()=>suggestDialog.showModal();
